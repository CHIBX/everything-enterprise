...
export default defineEventHandler(async (event) => {
    ...
    formData.forEach(async ({ name, filename, data }) => {
        ...
        if (!name || !filename || !ext.includes((filenameExt = filename.split(".").at(-1) as string))) {
            return;
        }
        ...
        try {
            ...
            const uploadResult$1 = new Promise<UploadApiResponse>((resolve, reject) => {
                cloudinary.uploader
                    .upload_stream({ unique_filename: true, discard_original_filename: true, folder: joinImagePath(folderName || ""), type: "upload", async: true, format: "webp" }, (error, uploadResult) => {
                        if (uploadResult) {
